...
import jetbrick.dao.dialect.spi.*;
import org.apache.commons.lang3.StringUtils;

public abstract class Dialect {
	private static final Map<String, Dialect> factorys = new HashMap<String, Dialect>();
	...
	static {
		...
		factorys.put(MySqlDialect.NAME.toLowerCase(), mysql);
		factorys.put(mysql.getName().toLowerCase(), mysql);

		SqlServerDialect sqlserver = new SqlServerDialect();
		factorys.put(SqlServerDialect.NAME.toLowerCase(), sqlserver);
		factorys.put(sqlserver.getName().toLowerCase(), sqlserver);

		OracleDialect oracle = new OracleDialect();
		factorys.put(OracleDialect.NAME.toLowerCase(), oracle);
		factorys.put(oracle.getName().toLowerCase(), oracle);

		H2Dialect h2 = new H2Dialect();
		factorys.put(H2Dialect.NAME.toLowerCase(), h2);
		factorys.put(h2.getName().toLowerCase(), h2);
	...
	public static Dialect getDialect(String name) {
		Dialect dialect = (name == null) ? null : factorys.get(name.toLowerCase());
		if (dialect == null) {
